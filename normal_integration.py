...
def bilateral_normal_integration(normal_map,
    ...
    :return: depth_map: The resulting depth map after the bilateral normal integration process.
             surface: A trimesh mesh representing the 3D surface reconstructed from the depth map.
    ...
    quads = facets[:, 1:].astype(np.int64)

    # Optional: choose triangulation per-quad by the shorter diagonal to reduce sliver triangles
    def triangulate_quads(quads, vertices, use_shortest_diagonal=True):
        if not use_shortest_diagonal:
            # Fixed split: (0,1,2) and (0,2,3)
            t1 = quads[:, [0, 1, 2]]
            t2 = quads[:, [0, 2, 3]]
        ...
        # Compare squared diagonals |AC|^2 vs |BD|^2
        diag_ac = np.einsum('ij,ij->i', a - c, a - c)
        diag_bd = np.einsum('ij,ij->i', b - d, b - d)
        use_ac = diag_ac <= diag_bd

        tris = np.empty((quads.shape[0] * 2, 3), dtype=np.int64)

        # Split along AC
        idx = np.where(use_ac)[0]
        tris[2 * idx + 0] = quads[idx][:, [0, 1, 2]]
        tris[2 * idx + 1] = quads[idx][:, [0, 2, 3]]

        # Split along BD
        idx = np.where(~use_ac)[0]
        tris[2 * idx + 0] = quads[idx][:, [1, 2, 3]]
        tris[2 * idx + 1] = quads[idx][:, [1, 3, 0]]

        return tris

    # Triangulate and build the mesh
    faces = triangulate_quads(quads, vertices, use_shortest_diagonal=True)
